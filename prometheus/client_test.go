...
func TestGetServiceMetrics(t *testing.T) {
	...
	assert.Equal(t, 0.3, metrics["request_size_out"].(*MetricHistogram).NinetyFiveP)
	assert.Equal(t, 0.4, metrics["request_size_out"].(*MetricHistogram).NinetyNineP)
	assert.IsType(t, &MetricHistogram{}, metrics["request_duration_out"])
	assert.Equal(t, 0.5, metrics["request_duration_out"].(*MetricHistogram).NinetyNineP)
	assert.IsType(t, &MetricHistogram{}, metrics["response_size_out"])
	assert.Equal(t, 0.6, metrics["response_size_out"].(*MetricHistogram).NinetyNineP)
	assert.IsType(t, &MetricHistogram{}, metrics["request_size_in"])
	assert.Equal(t, 0.7, metrics["request_size_in"].(*MetricHistogram).NinetyNineP)
	assert.IsType(t, &MetricHistogram{}, metrics["request_duration_in"])
	assert.Equal(t, 0.8, metrics["request_duration_in"].(*MetricHistogram).NinetyNineP)
	assert.IsType(t, &MetricHistogram{}, metrics["response_size_in"])
	assert.Equal(t, 0.9, metrics["response_size_in"].(*MetricHistogram).NinetyNineP)
}
