...
ln -s $KIALI_SOURCES kiali/operator
...
[NOTE]
If you are using `minikube` it's recommended that you enable the `registry` and `ingress` add-ond. The `k8s-minikube.sh` script should do this for you.
