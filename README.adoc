...
Configuration lives under the `chat_ai` section in link:./config/config.go[config/config.go]. At a minimum, set a default provider and define its models. Keys can be set directly for development or as secrets using the `secret:<secretName>:<secretKey>` pattern.
...
chat_ai:
  ...
  - name: openai-default
    ...
    key: "secret:my-ai-creds:apiKey"
    models:
    - name: gpt-4o-mini
      model: gpt-4o-mini
      enabled: true
      # key: "secret:my-ai-creds:apiKey" # optional per-model override
----
...
For example, first create the secret containing your API key:
...
Then configure `chat_ai` to reference the secret. The Kiali Operator and Helm charts automatically mount secrets referenced in `chat_ai` configuration - no need to configure `deployment.custom_secrets`:
...
Store in AI is required to keep context retention (Enabled by default). See link:./design/KEPS/ai-store/proposal.md[AI STORE README] for details.
