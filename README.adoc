...
auth:
  ...
  ldap:
    ...
    ldapUserFilter: (cn=%s)
    ldapGroupFilter: (cn=%s)
    ldapAttributes: ['memberOf', 'cn', 'mail']
    ldapBase: DC=example,DC=com
    ldapBindDN: CN={USERID},OU=xyz,OU=Users,OU=Accounts,DC=example,DC=com
    ldapSearchFilter: (&(name={USERID}))
    # Time for token expiration
    ldapTokenExpirationMin: 1440
