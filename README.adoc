...
To install the MCP (Model Context Protocol) server and OpenShift LightSpeed in a cluster for use with the AI integration, run the script `link:./hack/install-mcp-lightspeed.sh[hack/install-mcp-lightspeed.sh]`. It deploys the MCP server (Kubernetes or OpenShift variant), creates the LightSpeed operator subscription and namespace, and applies the OLS config. Usage: pass `MCP=kubernetes` or `MCP=openshift` and `API_TOKEN=your_token` (and optionally `MCP_IMAGE=...`). For full deployment steps (including CRC), see link:./ai/providers/lightspeed/DEPLOY.md[LightSpeed deploy guide].
