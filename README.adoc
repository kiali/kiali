...
|`MAISTRA`
|Istio installation is Maistra. (Default: false)
[source,yaml]
----
maistra: VALUE
----
...
|`JAEGER_SERVICE_PORT`
|The port of the Jaeger service.(default is 16686). Kiali uses JAEGER_SERVICE_PORT to connect internally (within the cluster) to Jaeger service to query the error traces.
[source,yaml]
----
external_services:
  jaeger:
