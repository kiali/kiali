...
func Get(ctx context.Context, conf *config.Config, clientFactory kubernetes.ClientFactory, cache cache.KialiCache, grafana *grafana.Service, prom prometheus.ClientInterface) StatusInfo {
	...
	info.ExternalServices = getVersions(ctx, conf, clientFactory, grafana, prom)
