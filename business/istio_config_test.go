...
func mockGetIstioConfigList(t *testing.T) IstioConfigService {
	...
	return IstioConfigService{userClients: k8sclients, kialiCache: cache, businessLayer: NewWithBackends(kubernetes.ConvertToUserClients(k8sclients), k8sclients, nil, nil), conf: conf}
}
...
func TestListWithAllNamespacesButNoAccessReturnsEmpty(t *testing.T) {
	...
	k8sclients := make(map[string]kubernetes.ClientInterface)
	k8sclients[conf.KubernetesConfig.ClusterName] = k8s
	configService := NewWithBackends(kubernetes.ConvertToUserClients(k8sclients), k8sclients, nil, nil).IstioConfig
