...
func (in *IstioValidationsService) GetValidations(ctx context.Context, namespace, service, workload string) (models.IstioValidations, error) {
	...
	skipAutogeneratedConfigs := config.Get().KialiFeatureFlags.Validations.SkipAutoGeneratedConfigs

	// We fetch without target service as some validations will require full-namespace details
	go in.fetchIstioConfigList(ctx, &istioConfigList, &mtlsDetails, &rbacDetails, namespace, errChan, &wg, skipAutogeneratedConfigs)
...
func (in *IstioValidationsService) fetchIstioConfigList(ctx context.Context, rValue *models.IstioConfigList, mtlsDetails *kubernetes.MTLSDetails, rbacDetails *kubernetes.RBACDetails, namespace string, errChan chan error, wg *sync.WaitGroup, filterAutogenerated bool) {
	defer wg.Done()
	if len(errChan) > 0 {
		return
	}
	...
	// Filter DR
	dr := istioConfigList.DestinationRules
	if filterAutogenerated == true {
		dr = kubernetes.FilterAutogeneratedDestinationRules(dr)
	}
	filteredDRs := in.filterDRExportToNamespaces(namespace, dr)
