...
func (in *MeshService) resolveKialiNetwork() (string, error) {
	...
	if err != nil {
		// Don't return an error, as this may mean that Kiali is installed along the control plane.
	...
	if err != nil {
		// This does not return an error, because it's probably valid that the configmap does not have the "global" key.
	...
	if !ok {
		// This does not return an error, because it's valid that the configmap does not have the "network" key, which most
...
// named the same as in Kiali Cluster.
...
func resolveNetwork(clusterName string, parsedSecret *kubernetes.RemoteSecret) string {
	conf := config.Get()

	restConfig, restConfigErr := kubernetes.UseRemoteCreds(parsedSecret)
	...
	restConfig.BearerToken = parsedSecret.Users[0].User.Token
