...
func TestK8sGatewayReferences(t *testing.T) {
	...
	gatewayReferences := K8sGatewayReferences{
		...
		WorkloadsPerNamespace: map[string]models.Workloads{
			"bookinfo": {
				data.CreateWorkload("bookinfo", "bookinfo", map[string]string{config.GatewayLabel: "bookinfo"}),
			},
			"test": {
				data.CreateWorkload("test", "test", map[string]string{config.GatewayLabel: "bookinfo"}),
			}},
	}
