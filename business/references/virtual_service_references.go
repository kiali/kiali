...
func (n VirtualServiceReferences) getConfigReferences(vs *networking_v1.VirtualService) []models.IstioReference {
	...
	allGateways := getAllGateways(vs, n.IdentityDomain)
	// filter unique references
			...
			for _, httpRoute := range virtualService.Spec.Http {
				...
				if len(httpRoute.Route) > 0 {
					for _, dest := range httpRoute.Route {
						...
						host := dest.Destination.Host
						drHost := kubernetes.GetHost(host, dr.Namespace, n.Namespaces, n.IdentityDomain)
						vsHost := kubernetes.GetHost(dr.Spec.Host, virtualService.Namespace, n.Namespaces, n.IdentityDomain)
						if kubernetes.FilterByHost(vsHost.String(), vsHost.Namespace, drHost.Service, drHost.Namespace, n.IdentityDomain) {
...
func getAllGateways(vs *networking_v1.VirtualService, identityDomain string) []models.IstioReference {
	allGateways := make([]models.IstioReference, 0)
	namespace := vs.Namespace
	if len(vs.Spec.Gateways) > 0 {
		allGateways = append(allGateways, getGatewayReferences(vs.Spec.Gateways, namespace, identityDomain)...)
	}
			...
			if httpRoute != nil {
				for _, match := range httpRoute.Match {
					if match != nil && match.Gateways != nil {
						allGateways = append(allGateways, getGatewayReferences(match.Gateways, namespace, identityDomain)...)
					}
				}
			}
		}
...
func getGatewayReferences(gateways []string, namespace string, identityDomain string) []models.IstioReference {
	result := make([]models.IstioReference, 0)
	for _, gate := range gateways {
		gw := kubernetes.ParseGatewayAsHost(gate, namespace, identityDomain)
		if !gw.IsWildcard() {
			if gate == "mesh" {
