...
func (ics *IstioCertsService) GetCertsInfo() ([]models.CertInfo, error) {
	...
	cert, err := ics.getCertificateFromSecret(UserProvidedCASecret, "ca-cert.pem")
	if err == nil {
	...
	cert, err = ics.getCertificateFromSecret(IstioDefaultCASecret, "ca-cert.pem")
	if err != nil {
		return nil, err
	}
	return []models.CertInfo{cert}, nil
...
func (ics *IstioCertsService) getChironCertificates(certsConfig []certConfig) ([]models.CertInfo, error) {
	...
	for _, certConfig := range certsConfig {
		...
		go func(secretName string, dnsNames []string) {
			...
			cert, err := ics.getCertificateFromSecret(secretName, "cert-chain.pem")
			if err != nil {
				errChan <- err
				return
			}
