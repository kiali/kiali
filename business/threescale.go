...
func (in *ThreeScaleService) GetThreeScaleInfo() (models.ThreeScaleInfo, error) {
	...
	conf := config.Get()
	_, err2 := in.k8s.GetIstioObject(conf.IstioNamespace, kubernetes.Adapters, conf.Extensions.ThreeScale.AdapterName)
	if err2 != nil {
	...
	canCreate, canUpdate, canDelete := getPermissions(in.k8s, conf.IstioNamespace, kubernetes.Adapters)
...
func (in *ThreeScaleService) CreateThreeScaleHandler(body []byte) (models.ThreeScaleHandlers, error) {
	...
	go func() {
		defer wg.Done()
		_, errInstance = in.k8s.CreateIstioObject(kubernetes.ResourceTypesToAPI[kubernetes.Instances], conf.IstioNamespace, kubernetes.Instances, jsonInstance)
	}()
...
func (in *ThreeScaleService) UpdateThreeScaleHandler(handlerName string, body []byte) (models.ThreeScaleHandlers, error) {
	...
	_, err2 = in.k8s.UpdateIstioObject(kubernetes.ResourceTypesToAPI[kubernetes.Handlers], conf.IstioNamespace, kubernetes.Handlers, handlerName, jsonUpdatedHandler)
	if err2 != nil {
		return nil, err2
	}
...
func (in *ThreeScaleService) DeleteThreeScaleHandler(handlerName string) (models.ThreeScaleHandlers, error) {
	...
	err = in.k8s.DeleteIstioObject(kubernetes.ResourceTypesToAPI[kubernetes.Handlers], conf.IstioNamespace, kubernetes.Handlers, handlerName)
	if err != nil {
		return nil, err
	}

	instanceName := "threescale-authorization-" + handlerName
	err = in.k8s.DeleteIstioObject(kubernetes.ResourceTypesToAPI[kubernetes.Instances], conf.IstioNamespace, kubernetes.Instances, instanceName)
	if err != nil {
		return nil, err
	}

	// It should delete all Rules generated by Wizard that used the deleted handler
	rules, err := in.k8s.GetIstioObjects(conf.IstioNamespace, kubernetes.Rules, "kiali_wizard")
	if err != nil {
		return nil, err
	}
	...
	for _, ruleName := range ruleNamesToDelete {
		go func(ruleNameToDelete string) {
			defer wg.Done()
			if len(errChan) == 0 {
				err = in.k8s.DeleteIstioObject(kubernetes.ResourceTypesToAPI[kubernetes.Rules], conf.IstioNamespace, kubernetes.Rules, ruleNameToDelete)
				if err != nil {
					errChan <- err
				}
			}
...
func (in *ThreeScaleService) GetThreeScaleRule(namespace, service string) (models.ThreeScaleServiceRule, error) {
	...
	rule, err := in.k8s.GetIstioObject(conf.IstioNamespace, kubernetes.Rules, ruleName)
	if err != nil {
...
func (in *ThreeScaleService) CreateThreeScaleRule(namespace string, body []byte) (models.ThreeScaleServiceRule, error) {
	...
	conf := config.Get()
	_, errRule := in.k8s.CreateIstioObject(kubernetes.ResourceTypesToAPI[kubernetes.Rules], conf.IstioNamespace, kubernetes.Rules, jsonRule)
	if errRule != nil {
...
func (in *ThreeScaleService) UpdateThreeScaleRule(namespace, service string, body []byte) (models.ThreeScaleServiceRule, error) {
	...
	conf := config.Get()
	_, errRule := in.k8s.UpdateIstioObject(kubernetes.ResourceTypesToAPI[kubernetes.Rules], conf.IstioNamespace, kubernetes.Rules, ruleName, jsonRule)
	if errRule != nil {
		return models.ThreeScaleServiceRule{}, errRule
	}
...
func (in *ThreeScaleService) DeleteThreeScaleRule(namespace, service string) error {
	...
	conf := config.Get()
	ruleName := "threescale-" + namespace + "-" + service
	err = in.k8s.DeleteIstioObject(kubernetes.ResourceTypesToAPI[kubernetes.Rules], conf.IstioNamespace, kubernetes.Rules, ruleName)
	return err
}
