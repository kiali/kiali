...
// ResolveIdentityDomain returns the effective Istio identity domain for host/FQDN parsing
...
func (in *SvcService) buildKubernetesServices(ctx context.Context, svcs []core_v1.Service, pods []core_v1.Pod, istioConfigList models.IstioConfigList, onlyDefinitions bool, cluster string) []models.ServiceOverview {
	identityDomain := in.ResolveIdentityDomain(ctx, cluster)
	...
	for i, item := range svcs {
		...
		if !onlyDefinitions {
			...
			svcVirtualServices := kubernetes.FilterAutogeneratedVirtualServices(kubernetes.FilterVirtualServicesByService(istioConfigList.VirtualServices, item.Namespace, item.Name, identityDomain))
			svcDestinationRules := kubernetes.FilterDestinationRulesByService(istioConfigList.DestinationRules, item.Namespace, item.Name, identityDomain)
...
func (in *SvcService) buildServiceEntryOverviews(ctx context.Context, serviceEntries []*networking_v1.ServiceEntry, existingServices []core_v1.Service, namespace string, istioConfigList models.IstioConfigList, cluster string) []models.ServiceOverview {
	identityDomain := in.ResolveIdentityDomain(ctx, cluster)
	services := []models.ServiceOverview{}
	...
	for _, se := range serviceEntries {
		...
		for _, hostname := range se.Spec.Hosts {
			host := kubernetes.GetHost(hostname, se.Namespace, clusterNamespaces, identityDomain)
...
func (in *SvcService) GetServiceDetails(ctx context.Context, cluster, namespace, service, interval string, queryTime time.Time, includeValidations bool) (*models.ServiceDetails, error) {
	...
	identityDomain := in.ResolveIdentityDomain(ctx, cluster)
	s.VirtualServices = kubernetes.FilterAutogeneratedVirtualServices(kubernetes.FilterVirtualServicesByService(istioConfigList.VirtualServices, namespace, service, identityDomain))
	s.DestinationRules = kubernetes.FilterDestinationRulesByService(istioConfigList.DestinationRules, namespace, service, identityDomain)
	s.K8sHTTPRoutes = kubernetes.FilterK8sHTTPRoutesByService(istioConfigList.K8sHTTPRoutes, istioConfigList.K8sReferenceGrants, namespace, service, identityDomain)
	s.K8sGRPCRoutes = kubernetes.FilterK8sGRPCRoutesByService(istioConfigList.K8sGRPCRoutes, istioConfigList.K8sReferenceGrants, namespace, service, identityDomain)
