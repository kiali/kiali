...
func (in *SvcService) getServiceList(ctx context.Context, criteria ServiceCriteria, cluster string) (*models.ServiceList, error) {
	...
	if criteria.IncludeHealth {
		for i, sv := range services.Services {
			services.Services[i].Health, err = in.businessLayer.Health.GetServiceHealth(ctx, cluster, criteria.Namespace, sv.Name, criteria.RateInterval, criteria.QueryTime, sv.ParseToService())
			if err != nil {
...
func (in *SvcService) GetServiceDetails(ctx context.Context, cluster, namespace, service, interval string, queryTime time.Time) (*models.ServiceDetails, error) {
	...
	go func(ctx context.Context) {
		defer wg.Done()
		var err2 error
		hth, err2 = in.businessLayer.Health.GetServiceHealth(ctx, cluster, namespace, service, interval, queryTime, &svc)
		if err2 != nil {
			errChan <- err2
		}
	}(ctx)
