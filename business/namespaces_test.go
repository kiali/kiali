...
func setupNamespaceService(t *testing.T, k8s kubernetes.ClientInterface, conf *config.Config) NamespaceService {
	...
	return NewNamespaceService(cache, conf, discovery, k8sclients, kubernetes.ConvertToUserClients(k8sclients))
}
...
func TestMultiClusterGetNamespace(t *testing.T) {
	...
	nsservice := NewNamespaceService(cache, conf, discovery, clients, kubernetes.ConvertToUserClients(clients))
