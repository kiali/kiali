...
func TestExportMultiHostMatchCorrect(t *testing.T) {
	...
	assert.Empty(vals)
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.DestinationRules.String(), Namespace: "test2", Name: "rule2"}]
	assert.False(ok)
...
func validationExportAssertion(assert *assert.Assertions, vals models.IstioValidations, namespace, refNamespace, drName string, refNames []string) {
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.DestinationRules.String(), Namespace: namespace, Name: drName}]
	assert.True(ok)
...
func TestExportMultiHostMatchWildcardInvalid(t *testing.T) {
	...
	assert.NotEmpty(vals)
	validation, ok = vals[models.IstioValidationKey{ObjectType: kubernetes.DestinationRules.String(), Namespace: "test", Name: "rule1"}]
	assert.True(ok)
	assert.True(validation.Valid) // As long as it is warning, this is true
	assert.NotEmpty(validation.Checks)
	assert.Equal(models.WarningSeverity, validation.Checks[0].Severity)
...
func TestExportReviewsExample(t *testing.T) {
	...
	assert.NotEmpty(vals)
	assert.Equal(3, len(vals))
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.DestinationRules.String(), Namespace: "bookinfo3", Name: "reviews3"}]
	assert.True(ok)
...
func TestExportMultiServiceEntryInvalid(t *testing.T) {
	...
	assert.NotEmpty(vals)
	assert.Equal(2, len(vals))
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.DestinationRules.String(), Namespace: "test2", Name: "service-a2"}]
	assert.True(ok)
	assert.True(validation.Valid)
	assert.NotEmpty(validation.Checks)
	assert.Equal(models.WarningSeverity, validation.Checks[0].Severity)
