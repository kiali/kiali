...
func TestWithoutK8sGatewayGRPC(t *testing.T) {
	...
	route1 := vals[models.IstioValidationKey{ObjectType: "k8sgrpcroute", Namespace: "bookinfo", Name: "route1"}]
	assert.False(route1.Valid)
	assert.NoError(validations.ConfirmIstioCheckMessage("k8sroutes.nok8sgateway", route1.Checks[0]))
	route2 := vals[models.IstioValidationKey{ObjectType: "k8sgrpcroute", Namespace: "bookinfo", Name: "route2"}]
...
func TestWithoutServiceGRPC(t *testing.T) {
	...
	route2 := vals[models.IstioValidationKey{ObjectType: "k8sgrpcroute", Namespace: "bookinfo2", Name: "route2"}]
	assert.False(route2.Valid)
