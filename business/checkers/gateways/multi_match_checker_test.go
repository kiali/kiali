...
func TestCorrectGateways(t *testing.T) {
	...
	assert.Empty(vals)
	_, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "test", Name: "validgateway"}]
	assert.False(ok)
}
...
func TestCaseMatching(t *testing.T) {
	...
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "test", Name: "foxxed"}]
	assert.True(ok)
...
func TestSameHostPortConfigInDifferentNamespace(t *testing.T) {
	...
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "bookinfo", Name: "stillvalid"}]
	assert.True(ok)
	assert.True(validation.Valid)

	secValidation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "test", Name: "validgateway"}]
	assert.True(ok)
...
func TestWildCardMatchingHost(t *testing.T) {
	...
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "test", Name: "stillvalid"}]
	assert.True(ok)
	assert.True(validation.Valid)
...
func TestSameWildcardHostPortConfigInDifferentNamespace(t *testing.T) {
	...
	assert.NotEmpty(vals)
	assert.Equal(2, len(vals))
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "bookinfo2", Name: "bookinfo-gateway-auto-host-copy"}]
	assert.True(ok)
	assert.True(validation.Valid)

	secValidation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "bookinfo", Name: "bookinfo-gateway-auto-host"}]
	assert.True(ok)
	assert.True(secValidation.Valid)
...
func TestAnotherSubdomainWildcardCombination(t *testing.T) {
	...
	assert.NotEmpty(vals)
	assert.Equal(1, len(vals))
	validation, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "test", Name: "shouldnotbevalid"}]
	assert.True(ok)
	assert.True(validation.Valid)
}
...
func TestDuplicateGatewaysErrorCount(t *testing.T) {
	...
	assert.NotEmpty(vals)
	validgateway, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "test", Name: "validgateway"}]
	assert.True(ok)

	duplicatevalidgateway, ok := vals[models.IstioValidationKey{ObjectType: kubernetes.Gateways.String(), Namespace: "test", Name: "duplicatevalidgateway"}]
	assert.True(ok)
