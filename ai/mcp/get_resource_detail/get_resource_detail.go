...
func Execute(ctx context.Context, args map[string]interface{}, businessLayer *business.Layer, prom prometheus.ClientInterface, clientFactory kubernetes.ClientFactory, kialiCache cache.KialiCache, conf *config.Config, grafana *grafana.Service, perses *perses.Service, discovery *istio.Discovery) (interface{}, int) {
	// Extract parameters
...
func getResourceDetails(ctx context.Context, businessLayer *business.Layer, resourceArgs ResourceDetailArgs, namespace string) (interface{}, int, error) {
	...
	go func() {
		defer wg.Done()
		if resourceArgs.ResourceType == "service" {
		...
		} else if resourceArgs.ResourceType == "workload" {
