...
func (t ResourceDetailTool) Call(ctx context.Context, args map[string]interface{}, business *business.Layer, prom prometheus.ClientInterface, clientFactory kubernetes.ClientFactory, kialiCache cache.KialiCache, conf *config.Config, grafana *grafana.Service, perses *perses.Service, discovery *istio.Discovery) (interface{}, int) {
	return get_resource_detail.Execute(ctx, args, business, prom, clientFactory, kialiCache, conf, grafana, perses, discovery)
}
