...
func (t MeshGraphTool) Call(ctx context.Context, args map[string]interface{}, business *business.Layer, prom prometheus.ClientInterface, clientFactory kubernetes.ClientFactory, kialiCache cache.KialiCache, conf *config.Config, grafana *grafana.Service, perses *perses.Service, discovery *istio.Discovery) (interface{}, int) {
	return get_mesh_graph.Execute(ctx, args, business, prom, clientFactory, kialiCache, conf, grafana, perses, discovery)
}
