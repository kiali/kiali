...
## 2. list_or_get_resources

Returns a **list** of resources or **detailed** data for one resource. Omit `resourceName` to list; set `resourceName` for detail (requires **exactly one** namespace in `namespaces`, except `resourceType` `namespace` where `resourceName` is the namespace name).

```bash
curl -X POST "$KIALI_URL/api/chat/mcp/list_or_get_resources" \
  -H "Content-Type: application/json" \
  -d '{
...
|--------------|--------|----------|-------------|
| resourceType | string | Yes      | `service`, `workload`, `app`, `namespace` |
| namespaces   | string | No       | Comma-separated namespaces (empty = all accessible). With `resourceName`, must be a single namespace. |
| resourceName | string | No       | If empty, lists all; if set, returns detail for that resource |
...
Lists traces for a service in a namespace. Returns `summary` (namespace, service, total_found, avg_duration_ms) and `traces` (id, duration_ms, spans_count, root_op, slowest_service, has_errors). Use **get_trace_details** with a trace `id` to get the full call hierarchy.

```bash
curl -X POST "$KIALI_URL/api/chat/mcp/list_traces" \
  -H "Content-Type: application/json" \
  -d '{
    "namespace": "bookinfo",
...
| lookback_seconds | integer | How far back to search (default 600) |
| limit            | integer | Max traces to return (default 10) |

---

## 4. get_trace_details

Fetches a single trace by `trace_id` and returns its call hierarchy (tree of services with duration_ms, status, offset_ms, optional `direction`, and nested `calls`). Use an `id` from **list_traces** response. When the same service appears twice in a row (e.g. productpage → productpage → details), `direction` is set from `span.kind`: **outbound** = sidecar sending, **inbound** = service receiving, so the duplicate is the proxy hop.

```bash
curl -X POST "$KIALI_URL/api/chat/mcp/get_trace_details" \
  -H "Content-Type: application/json" \
  -d '{
    "trace_id": "21a90b5531039d94c85b047394016374"
  }'
```
...
| Parameter | Type   | Description |
|-----------|--------|--------------|
| trace_id  | string | Trace ID (required). From list_traces list response. |
...
| list_or_get_resources | List or detail for service, workload, app, namespace |
| list_traces         | List traces by namespace + service |
| get_trace_details   | Single trace call hierarchy by trace_id |
