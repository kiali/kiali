...
// HealthStatusMetricItem represents a single health status metric with its labels and value.
...
// HealthStatusMetrics holds all health status metrics.
type HealthStatusMetrics struct {
	Metrics []HealthStatusMetricItem `json:"metrics"`
}

// getGaugeVecValues extracts all values from a GaugeVec with their labels.
	...
	for metric := range ch {
		...
		if err := metric.Write(m); err == nil && m.Gauge != nil {
			...
			for _, label := range m.Label {
				...
				case "cluster":
					item.Cluster = label.GetValue()
				case "namespace":
					item.Namespace = label.GetValue()
				case "health_type":
					item.HealthType = label.GetValue()
				case "name":
					item.Name = label.GetValue()
				}
			}
...
// This endpoint provides a simple way to access the kiali_health_status metric values
// without having to scrape and parse the Prometheus text format from the /metrics endpoint.
