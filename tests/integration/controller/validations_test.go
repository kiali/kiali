...
var _ = Describe("Start", func() {
	var kialiCache cache.KialiCache
	Context("When validations are disabled", func() {
		BeforeEach(func(ctx SpecContext) {
			...
			err = controller.Start(ctx, conf, nil, nil, nil)
			Expect(err).ToNot(HaveOccurred())

			kialiCache.Validations().Replace(nil)
		})

		It("Should not validate any resources and validations should always be empty", func(ctx SpecContext) {
			Expect(kialiCache.Validations().Items()).Should(BeEmpty())
			...
			By("By checking that the validations are NOT created in the kiali cache")
			Eventually(func() bool {
				validations := kialiCache.Validations().Items()
				return len(validations) == 0
			}, timeout, interval).Should(BeTrue())
