...
func run(ctx context.Context, conf *config.Config, version string, commitHash string, goVersion string, staticAssetFS fs.FS, clientFactory kubernetes.ClientFactory, logger *zerolog.Logger) <-chan struct{} {
...
func Run(ctx context.Context, conf *config.Config, version string, commitHash string, goVersion string, staticAssetFS fs.FS, clientFactory kubernetes.ClientFactory, logger *zerolog.Logger) <-chan struct{} {
	...
	return run(ctx, conf, version, commitHash, goVersion, staticAssetFS, clientFactory, logger)
}
