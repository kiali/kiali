...
func FilterAutogeneratedDestinationRules(destinationRules []*networking_v1beta1.DestinationRule) []*networking_v1beta1.DestinationRule {
	...
	for _, dr := range destinationRules {
		// Autogenerated DRs by maistra don't have autogenerated in the name, but they add the owner label
		if !IsMaistraAutogenerated(dr.Labels) {
...
func IsMaistraAutogenerated(labels map[string]string) bool {
	return strings.Contains(labels["maistra.io/owner"], config.Get().IstioNamespace)
}
