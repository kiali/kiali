...
// handler is a generic type for handlers defined from Istio 1.1.0
// Preliminar rule is to define one file per type.
const handlers = "handlers"
const handlerType = "handler"
const handlerLabel = "handler"

// handler is the generic Kubernetes API object wrapper
// handler starts with lowercase as it maps a "kind":"handler" Istio response.
type handler struct {
	meta_v1.TypeMeta   `json:",inline"`
	meta_v1.ObjectMeta `json:"metadata"`
	Spec               map[string]interface{} `json:"spec"`
...
// handlerList is the generic Kubernetes API list wrapper
// handlerList starts with lowercase as it maps a "kind":"handlerList" Istio response.
...
// GetSpec from a wrapper
func (in *handler) GetSpec() map[string]interface{} {
	return in.Spec
}

// SetSpec for a wrapper
func (in *handler) SetSpec(spec map[string]interface{}) {
	in.Spec = spec
}

// GetObjectMeta from a wrapper
func (in *handler) GetObjectMeta() meta_v1.ObjectMeta {
	return in.ObjectMeta
}

// SetObjectMeta for a wrapper
func (in *handler) SetObjectMeta(metadata meta_v1.ObjectMeta) {
	in.ObjectMeta = metadata
}

// GetItems from a wrapper
func (in *handlerList) GetItems() []IstioObject {
	out := make([]IstioObject, len(in.Items))
	for i := range in.Items {
		out[i] = &in.Items[i]
	}
...
// DeepCopyIstioObject is an autogenerated deepcopy function, copying the receiver, creating a new IstioObject.
func (in *handler) DeepCopyIstioObject() IstioObject {
	if c := in.DeepCopy(); c != nil {
		return c
	}
