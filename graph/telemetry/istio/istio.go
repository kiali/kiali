...
func buildNamespaceTrafficMap(namespace string, o graph.TelemetryOptions, client *prometheus.Client) graph.TrafficMap {
	...
	query = fmt.Sprintf(`sum(rate(%s{reporter="%s",source_workload_namespace!="%s",source_workload!="unknown",destination_service_namespace="%s"} [%vs])) by (%s)`,
		requestsMetric,
		reporter,
		namespace,
		namespace,
	...
	if isIstio {
		...
		istioNamespacesRegex := strings.Join(appender.GetIstioNamespaces(o.Namespaces), "|")
