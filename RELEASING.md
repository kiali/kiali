...
## Post-Release Checklist and Troubleshooting

For a post-release verification checklist, troubleshooting, and recovery steps, see [RELEASE-CHECKLIST.md](./RELEASE-CHECKLIST.md).
...
The pipelines are scheduled to run on every Monday (except OSSMC which runs on Tuesday) detecting if a release is needed (on every Sprint ending a release will be generated). Also, they can be started manually filling some parameters required by the pipeline (release type, branch name and image name).
...
By design, the release pipelines can be triggered manually and also are scheduled to trigger automatically (Monday for most repos, Tuesday for OSSMC).
