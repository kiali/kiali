...
if [ "$_CMD" = "up" ]; then
  ...
  if [ "${KIALI_ENABLED}" == "true" ]; then
    ...
    while [ "$(oc get deployment.apps -n istio-system -o jsonpath='{range .items[*]}{.metadata.name}{" "}{end}' | wc -w)" == "0" ]
    do
      sleep 5
      echo -n '.'
    done
    ...
    for app in $(oc get deployment.apps -n istio-system -o jsonpath='{range .items[*]}{.metadata.name}{" "}{end}')
    do
