...
# For minikube, we don't need to worry about node images like with Kind
MINIKUBE_NODE_IMAGE=""
...
MINIKUBE_NODE_IMAGE=$MINIKUBE_NODE_IMAGE
...
setup_minikube_multicluster() {
  ...
  if [ "${MULTICLUSTER}" == "${EXTERNAL_CONTROLPLANE}" ]; then
    "${SCRIPT_DIR}"/istio/multicluster/setup-external-controlplane-minikube.sh ${istio_version_arg}
    ...
    echo "ERROR: Minikube multicluster setup currently only supports external-controlplane. Aborting."
    exit 1
  fi
  ...
  "${SCRIPT_DIR}"/istio/multicluster/deploy-kiali-minikube.sh \
...
if [ -n "${MULTICLUSTER}" ]; then
  ...
  echo "ERROR: Minikube setup currently only supports multicluster configurations. Aborting."
  exit 1
fi
