...
# This scripts combines traces from the west cluster into the east cluster by sending traces through the istio-ingressgateway.
...
ingress_output=$(kubectl get svc -n istio-system --context "${CLUSTER1_CONTEXT}" istio-ingressgateway -o jsonpath='{.spec.ports[?(@.name=="zipkin-http")]}')
# Check if the output is empty
if [ -z "$ingress_output" ]; then
    kubectl --context "${CLUSTER1_CONTEXT}" patch Service -n istio-system istio-ingressgateway --type=json -p '[{"op": "add", "path": "/spec/ports/-", "value": {"name": "zipkin-http", "port": 9411, "protocol": "TCP", "targetPort": 8080}}]'
fi
...
ISTIO_INGRESS_IP=$(kubectl --context "${CLUSTER1_CONTEXT}" get service -n istio-system istio-ingressgateway -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
