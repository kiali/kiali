...
if [ "${OLM_ENABLED}" == "true" ]; then
  if [ "${OLM_VERSION}" == "latest" ]; then
    ...
    if [ -z "${OLM_VERSION}" ]; then
      infomsg "Failed to obtain the latest OLM version from GitHub after 120 attempts over 2 hours."
      exit 1
    else
  ...
  if [ -z "${olm_install_script:-}" ]; then
    infomsg "Failed to download OLM install script from GitHub after 120 attempts over 2 hours."
    exit 1
  fi
  ...
  infomsg "Running OLM install script..."
  ...
  for env_name in ALLOW_AD_HOC_KIALI_NAMESPACE ALLOW_AD_HOC_KIALI_IMAGE ALLOW_AD_HOC_INIT_CONTAINERS ALLOW_SECURITY_CONTEXT_OVERRIDE; do
