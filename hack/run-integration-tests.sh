...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -ts|--test-suite)
      TEST_SUITE="${2}"
      if [ "${TEST_SUITE}" != "${BACKEND}" -a "${TEST_SUITE}" != "${BACKEND_EXTERNAL_CONTROLPLANE}" -a "${TEST_SUITE}" != "${FRONTEND}" -a "${TEST_SUITE}" != "${FRONTEND_AMBIENT}" -a "${TEST_SUITE}" != "${FRONTEND_PRIMARY_REMOTE}" -a "${TEST_SUITE}" != "${FRONTEND_MULTI_PRIMARY}" -a "${TEST_SUITE}" != "${FRONTEND_TEMPO}" -a "${TEST_SUITE}" != "${LOCAL}" ]; then
        echo "--test-suite option must be one of '${BACKEND}', '${BACKEND_EXTERNAL_CONTROLPLANE}', '${FRONTEND}', '${FRONTEND_PRIMARY_REMOTE}', '${FRONTEND_MULTI_PRIMARY}', '${FRONTEND_AMBIENT}', '${FRONTEND_TEMPO}', or '${LOCAL}'"
        exit 1
      fi
      shift;shift
      ;;
...
Valid command line arguments:
  ...
  -to|--tests-only <true|false>
    ...
    Default: false
  -ts|--test-suite <${BACKEND}|${BACKEND_EXTERNAL_CONTROLPLANE}|${FRONTEND}|${FRONTEND_AMBIENT}|${FRONTEND_PRIMARY_REMOTE}|${FRONTEND_MULTI_PRIMARY}|${FRONTEND_TEMPO}|${LOCAL}>
...
EOF
  ...
  infomsg "Starting Kiali locally using binary: ${KIALI_BINARY}"
  ...
  if [ "${SETUP_ONLY}" == "true" ]; then
    infomsg "Setup complete. Kiali is running locally at ${KIALI_URL} (PID: ${KIALI_PID})"
    infomsg "To stop Kiali, run: kill ${KIALI_PID}"
    exit 0
  fi
