...
if [ "${TEST_SUITE}" == "${BACKEND}" ]; then
  ...
  if [ "${MCP_TOOLS}" == "true" ]; then
    ...
    go test -tags exclude_frontend -v -failfast -coverprofile=coverage-integration-mcp.out 2>&1
  else
    # Run backend multicluster integration tests
    cd "${SCRIPT_DIR}"/../tests/integration/tests
    go test -v -failfast -coverprofile=coverage-integration-multi-cluster.out 2>&1 | tee >(go-junit-report > ../junit-rest-report.xml) ../int-test.log
    detectRaceConditions
  fi
