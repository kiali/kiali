...
setup_kind_singlecluster() {
  ...
  if [ -n "${AMBIENT}" ]; then
      ...
      GAE="false"
      unset LOAD_BALANCER
      "${SCRIPT_DIR}"/istio/install-istio-via-istioctl.sh --client-exe-path "$(which kubectl)" -cn "cluster-default" -cn "cluster-default" -mid "mesh-default" -net "network-default" ${hub_arg:-} -cp ambient
  else
