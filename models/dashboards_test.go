...
func TestPrepareIstioDashboard(t *testing.T) {
	...
	assert.Equal(Aggregation{Label: "destination_canonical_service", DisplayName: "Remote app"}, dashboard.Aggregations[2])
	assert.Equal(Aggregation{Label: "destination_canonical_revision", DisplayName: "Remote version"}, dashboard.Aggregations[3])
	assert.Equal(Aggregation{Label: "response_code", DisplayName: "Response code"}, dashboard.Aggregations[4])
	assert.Equal(Aggregation{Label: "grpc_response_status", DisplayName: "GRPC status"}, dashboard.Aggregations[5])
	assert.Equal(Aggregation{Label: "response_flags", DisplayName: "Response flags"}, dashboard.Aggregations[6])
}
