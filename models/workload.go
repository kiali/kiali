...
type WorkloadListItem struct {
	...
	// Define if this Workload is Argo Rollout
	// required: true
	// example: true
...
func (workload *WorkloadListItem) ParseWorkload(w *Workload, conf *config.Config) {
	...
	workload.IsRollout = w.HasRollout()
...
// MergeReplicaSets for a case or ArgoRollouts, the Workload should be read only and not fully supported
func (workload *Workload) MergeReplicaSets(rs []*apps_v1.ReplicaSet) {
	workload.WorkloadGVK = kubernetes.ReplicaSets
	// this will merge labels from replica sets which are generated by Rollout and each has own template hash which will be ignored
	for _, r := range rs {
...
// IsRollout return true if the workload is a generated by Rollout (Based on labels)
func (workload *Workload) HasRollout() bool {
	for _, pod := range workload.Pods {
		if pod.Labels[util.RolloutsLabel] != "" {
			return true
		}
	}
	return false
