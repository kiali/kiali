...
# Install kiali-ui dependencies (corepack will automatically use the correct Yarn version)
yarn install
...
We use Yarn 4 (managed via corepack) as the package manager. If adding dependencies
to `package.json`, make sure you install them with `yarn` and commit the `yarn.lock` file.
