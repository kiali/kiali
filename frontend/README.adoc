...
Use the `useTranslation` hook to get the `t` translation function. More info link:https://react.i18next.com/latest/usetranslation-hook[here]. 
...
[source, typescript]
----
export const StatefulFilters = withTranslation(I18N_NAMESPACE, { withRef: true })(StatefulFilters);
----
