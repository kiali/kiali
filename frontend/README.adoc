...
The dev server proxy is configured in `rsbuild.config.ts` and defaults to `http://localhost:20001/kiali`.
...
[source, shell]
----
make KIALI_PROXY_URL=http://localhost:20001/kiali run-frontend
----

If Kiali is configured with a different web root, adjust the URL accordingly.
