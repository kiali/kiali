...
For string literals that are stored in a variable you have to translate the variable itself within the React component (class or hook). 
...
Include i18n t function as input parameter (must be imported from `react-i18next` library, it will fail if imported from `i18next` library):
