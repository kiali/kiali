...
When('user deletes a Traffic Policy and the resource is no longer available in any cluster',() => {
  const authorizationPolicies: string[] = ['deny-all-bookinfo', 'details-v1', 'productpage-v1','ratings-v1','reviews-v1','reviews-v2','reviews-v2'];
  const sidecars: string[] = ['kiali-traffic-generator', 'details-v1', 'productpage-v1','ratings-v1','reviews-v1','reviews-v2','reviews-v2'];
...
When('user decides to create a Traffic Policy in the {string} {string}',(cluster:string,ns:string) =>{
  cy.get(`[data-test="CardItem_${ns}_${cluster}"]`).find('[aria-label="Actions"]').click();
  cy.get('button').contains('Create Traffic Policies').should('be.visible');
  cy.get('button').contains('Create Traffic Policies').click();
});
