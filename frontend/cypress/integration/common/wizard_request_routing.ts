import { And, Before, Given, When } from 'cypress-cucumber-preprocessor/steps';
...
When('user clicks in the {string} actions', action => {
  ...
  cy.get('button[data-test="wizard-actions"]').click().get('#loading_kiali_spinner').should('not.exist');
...
And('user creates the configuration', () => {
  ...
  cy.get('button[data-test="confirm-create"]').click().get('#loading_kiali_spinner').should('not.exist');
});

And('user updates the configuration', () => {
  cy.get('button[data-test="update"]').click();

  cy.get('button[data-test="confirm-update"]').click().get('#loading_kiali_spinner').should('not.exist');
});

And('user confirms delete the configuration', () => {
  cy.get('button[data-test="confirm-delete"]').click().get('#loading_kiali_spinner').should('not.exist');
});
