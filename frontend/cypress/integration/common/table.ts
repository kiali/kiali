...
export const getClusterForSingleCluster = (): string => {
...
export const checkHealthIndicatorInTable = (
  ...
  const cluster = getClusterForSingleCluster();
...
Then(
  ...
  (name: string, object: string, cluster: string, ns: string) => {
    cy.get(`[data-test="VirtualItem_Cluster${cluster}_Ns${ns}_${object}_${name}"]`).should('not.exist');
  }
);
