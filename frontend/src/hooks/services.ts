...
const useServiceDetail = (
  ...
  React.useEffect(() => {
    ...
    let getGwPromise = API.getAllIstioConfigs([gvkToString(dicIstioTypeToGVK['Gateway'])], false, '', '', cluster);
    let getPeerAuthsPromise = API.getIstioConfig(
      namespace,
    ...
    allPromise.promise
      .then(results => {
        ...
        setGateways(
          getGatewaysAsList(
            filterAutogeneratedGateways(results[1].data.resources[gvkToString(dicIstioTypeToGVK['Gateway'])])
          ).sort()
        );

        setPeerAuthentications(results[2].data.resources[gvkToString(dicIstioTypeToGVK['PeerAuthentication'])]);
        setFetchError(null);
        setIsLoading(false);
      })
      .catch(error => {
