...
const useServiceDetail = (
  ...
  React.useEffect(() => {
    ...
    let getGwPromise = API.getAllIstioConfigs([getGVKTypeString(gvkType.Gateway)], false, '', '', cluster);
    ...
    allPromise.promise
      .then(results => {
        ...
        setPeerAuthentications(results[2].data.resources[getGVKTypeString(gvkType.PeerAuthentication)]);
        setFetchError(null);
        setIsLoading(false);
      })
      .catch(error => {
...
export const useServiceDetailForGraphNode = (
  ...
  React.useEffect(() => {
    if (fetchError !== null) {
      addError('Could not fetch Service Details.', fetchError);
    }
  }, [fetchError]);
