...
export const INITIAL_CHAT_AI_STATE: ChatAIState = ImmutableMap({
...
export const ChatAiStateReducer = (state: ChatAIState = INITIAL_CHAT_AI_STATE, action: KialiAppAction): ChatAIState => {
  switch (action.type) {
    ...
    case getType(ChatAIActions.chatHistoryPush): {
      return state.set('chatHistory', state.get('chatHistory').push(ImmutableMap(action.payload.entry)));
    }
    case getType(ChatAIActions.chatHistoryUpdateByID): {
      const index = state.get('chatHistory').findIndex(entry => entry.get('id') === action.payload.id);
      return state.mergeIn(['chatHistory', index], action.payload.entry);
    }
    case getType(ChatAIActions.chatHistoryUpdateTool): {
      const index = state.get('chatHistory').findIndex(entry => entry.get('id') === action.payload.id);
      return state.mergeIn(['chatHistory', index, 'tools', action.payload.toolID], action.payload.tool);
    }
