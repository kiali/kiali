...
const nodeContextMenu = (node: GraphElement, kiosk: string): Promise<React.ReactElement[]> => {
  ...
  const items = optionsPF.map((o, i) => {
    return (
      ...
      !!o.altClickHandler ? (
        ...
        <ContextMenuItem className={contextMenuStyle} key={`option-${i}`} onClick={() => clickHandler(o, kiosk)}>
  ...
  let getDetailPromise = API.getServiceDetail(nodeData.namespace, nodeData.service!, false, nodeData.cluster);
  let getGwPromise = API.getAllIstioConfigs(['gateways'], false, '', '', nodeData.cluster);
  ...
  return new Promise<React.ReactElement[]>((resolve, reject) => {
    ...
    allPromise.promise
      .then(results => {
        ...
        const gateways = getGatewaysAsList(filterAutogeneratedGateways(results[1].data.gateways)).sort();
        const peerAuthentications = results[2].data.peerAuthentications;
