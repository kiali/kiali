...
type EmptyNamespacesState = {};

export class EmptyNamespaces extends React.Component<EmptyNamespacesProps, EmptyNamespacesState> {
  render(): React.ReactNode {
    if (this.props.refreshInterval === RefreshIntervalManual && !this.props.loaded) {
      return (
        ...
        >
          <EmptyStateBody>
            {t(
              'The refresh interval is set to "Manual". To render the namespaces, select your desired filters and options and then click the Refresh button. Or, if preferred, change the setting to the desired interval.'
            )}
          </EmptyStateBody>
        </EmptyState>
      );
    ...
    if (this.props.filteredNamespaces.length === 0) {
      return (
        <EmptyState
          headingLevel="h5"
          titleText={t('No unfiltered namespaces')}
          id="empty-vl-manual"
          variant={EmptyStateVariant.lg}
          className={emptyStateStyle}
        >
