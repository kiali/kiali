...
export class ValidationSummary extends React.PureComponent<Props> {
  ...
  tooltipNA() {
    return (
      <Text className={tooltipSentenceStyle}>{$t('Istio.NoIstioConfigObjects', 'No Istio config objects found')}</Text>
    );
  }

  tooltipNoValidationAvailable() {
    return (
      <Text className={tooltipListStyle}>
        {$t('Istio.NoIstioConfigValidation', 'No Istio config validation available')}
      </Text>
    );
  }
    ...
    return (
      <>
        <Text style={{ textAlign: 'left', textEmphasis: 'strong' }} component={TextVariants.p}>
          {$t('Istio.IstioConfigObjectsAnalyzed', 'Istio config objects analyzed')}: {this.props.objectCount}
        </Text>
        ...
        {this.props.reconciledCondition?.status && (
          <Text style={{ textAlign: 'left', textEmphasis: 'strong' }} component={TextVariants.p}>
            {$t('objectReconciliationStatus', 'The object is reconciled')}
          </Text>
        )}
      </>
    );
