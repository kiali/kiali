...
export const ChatHistoryEntry = React.memo(({ conversationID, entryIndex }: ChatHistoryEntryProps) => {    
    ...
    const query: string = useSelector((s: KialiAppState) =>  s.aiChat.get('chatHistory').get(entryIndex - 1).get('text'));
    const response: string = useSelector((s: KialiAppState) =>
        s.aiChat.get('chatHistory').get(entryIndex).get('text'),
    );
