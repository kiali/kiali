...
export type MockToolCall = { args?: Record<string, unknown>; id: string; name: string };
export type MockToolResult = { content: string; id: string; status: string };

export type MockStreamPayload = {
  actions?: unknown[];
  answer?: string;
  referenced_docs?: unknown[];
 ...
 * Leaves the chat entry with isStreaming:true after the body is consumed so the stop button stays visible.
 */
export function createMockStreamResponseNoEnd(payload: MockStreamPayload): string {
  let stream = '';
...
export function createMockStreamResponse(payload: MockStreamPayload): string {
  let stream = '';
  stream += `data: ${JSON.stringify({ event: 'start', data: { conversation_id: 'mock-conv-id' } })}\n\n`;
...
/** Minimal SSE stream with a single `error` event instead of a successful `end`. */
...
/** Resource name used by YAML create/patch/delete scenarios. */
export const AI_CHATBOT_TEST_VS = 'vs-ai-pw';

export const SECOND_PROVIDER_NAME = 'second-test-provider';
